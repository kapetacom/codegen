...
const ASSETS_FILE       = '.blockware/generated-files.yml';
...
class CodeWriter {
    ...
    /**
     * Reads the Blockware assets bookkeeping file that allows us to
    ...
    _cleanupAssets(oldAssets) {
        ...
        oldAssets.forEach((asset) => {
            ...
            if (canDelete) {
                const fullPath = Path.join(this._baseDir, asset.filename);
                if (FS.existsSync(fullPath)) {
                    console.log('Cleaning up unused blockware asset file: %s', asset.filename);
